...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (func_name == "lower")
  {
    auto result = std::make_shared<StringExpression>(args[0], StringExpressionType::Lower);
    return result;
  }
  if (func_name == "upper")
  {
    auto result = std::make_shared<StringExpression>(args[0], StringExpressionType::Upper);
    return result;
  }
